...
```text
41. Application builds are rootless and associated with the site's build UID;
    Docker runtime containers are controlled only by root.
...
For Docker runtime sites, the conventional rootful Docker daemon starts a
BonesDeploy-generated application container. The application has no engine
socket, no privileged mode, no host-root mount, and no capabilities by
default. Its active release is read-only; only declared shared and runtime
socket paths are writable. Rootful daemon control is a BonesDeploy
infrastructure operation, not an application or build-user capability.

...
A Rust CLI that compiles into a single binary, containing embeds of boilerplate scripts along with other git remote helpers.

## Deployment methodology. 
We have an ssh deployment user, (`git` normally) that handles deployment concerns. This user has a home folder and has sudo ability, but no password login. We also have a service user: `applications`. This user has no home folder, no login, and no sudo ability. This is ultimately who we want to own our project files to limit attack scope.
...
We create a `gitbones-remote` executable that does not require a password and allows it to change ownership to a deploy user and harden back the permissions based on what is configured under `permissions` in bones.toml.
...
- `pre-receive` => This runs `gitbones-remote doctor` and fails if there are any warnings or errors. Then it runs `gitbones-remote`

Runs Changes the permissions of the worktree to be owned by the ssh_user, which is necessary in order to run deployment scripts from this user. There will be a small subset of commands that are set in the sudoer's file (more on this later).
- `post-receive` => Update our git worktree to the lastest commit, (standard git deployment flow). Then it will call our custom git-hook, `deploy`.
...
### Deployment Folder
This folder stores deployment scripts to be called by `deploy`. Files in this folder must be ordered sequentially like `01_run_deployment_concerns.sh` and `02_lockup_permissions.sh`. These essentially run like database migrations but for deployment concerns.

## Crate structure
There will be two executables: `gitbones` and `gitbones-remote`. This keeps concerns separate and allows for installing only what you need on remote. `gitbones-remote` handles only a few necessary operations for ensuring that everything is setup properly.
...
- **init**:
  ...
  - Syncs local `dev/post-deployment.sh` to remote bare repo.
  - Builds and uploads post-receive hook to remote.
  - Saves config to root `bones.json`.
...
- User can make any changes to their
- User runs `git push production master` or some similar command where the remote name aligns with our .bones.toml configuation.
- The `pre-push` hook checks to see if we are pushing to our bones remote, in this example, that will be `production`.
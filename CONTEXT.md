...
### Update Patches
`bonesdeploy update` runs ordered Rust migration patches after each local or remote binary update. Completed patches are recorded per project and scope, so interrupted updates retry safely without rerunning successful patches. Local patches run in `bonesdeploy` and use the project data directory; remote patches run in `bonesremote`, use `/var/lib/bonesdeploy/patches/<site>/`, and are requested through the root SSH session. `--skip-local` and `--skip-remote` also skip their respective patches.

...
The optional git push transport uses thin adapters: a local `pre-push` guard (installed by `bonesdeploy init`) and a remote `post-receive` trigger (installed by provisioning). The config repo uses a separate `config-post-receive` trigger that calls `bonesremote site receive`. You do not see or manage them under `.bones/`. Set `deploy_on_push = true` in `.bones/bones.toml` to enable git-triggered deploys; the default is `false`.

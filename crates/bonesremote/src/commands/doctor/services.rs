...
fn check_target_membership(target: &str, project_root: &str, issues: &mut Vec<String>) {
    ...
    for service in services {
        check_required_service_active(target, &service, project_root, issues);
    }
}

fn check_required_service_active(target: &str, service: &str, project_root: &str, issues: &mut Vec<String>) {
    match systemd::active_status(service) {
        Ok(true) => {}
        Ok(false) if is_deferred_service(project_root, service) => {}
...
fn is_deferred_service(project_root: &str, service: &str) -> bool {
    current_is_placeholder(project_root) && condition_failed(service)
}

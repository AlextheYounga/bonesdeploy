...
mod tests {
    ...
    fn runtime_defaults_fit_the_single_file_schema() -> anyhow::Result<()> {
        for runtime in runtime_names() {
            ...
            let config: Runtime = serde_json::from_value(Value::Object(defaults))?;
    ...
    fn runtime_answers_accept_boolean_template_settings() -> anyhow::Result<()> {
        ...
        answers.insert("static".into(), Value::Bool(true));

        let config: Runtime = serde_json::from_value(Value::Object(answers))?;
        assert!(config.extra.get("static").is_some_and(|value| value.as_bool() == Some(true)));

...
def deploy(ctx):
    def seed_placeholder(current_ctx, paths, python_binary):
        ...
        mkdir(name="Ensure placeholder config directory exists", path=f"{placeholder}/config", user="root", group=current_ctx.runtime.runtime_group, mode="0750")
        render("Seed placeholder WSGI application", TEMPLATES / "django/placeholder-wsgi.py.j2", f"{placeholder}/config/wsgi.py", user="root", group=current_ctx.runtime.runtime_group, mode="0640", **template_data(current_ctx, paths=paths))
    ...
    def validate(current_ctx, paths, _python_binary):
        ...
        validation.run_as_runtime_user(current_ctx, "Validate Gunicorn configuration as runtime user", f"{gunicorn} --check-config {module}")

    def command(current_ctx, paths, _python_binary):
        module = current_ctx.runtime.data.get("wsgi_module", "config.wsgi:application")
        return f"{paths['current']}/.venv/bin/gunicorn {module} --bind unix:{paths['runtime_socket_dir']}/gunicorn/gunicorn.sock"

...
def deploy(ctx):
    ...
    def seed_placeholder(current_ctx, paths, ruby_binary):
        ...
        server.shell(name="Install placeholder gems", commands=[f"cd {quote(placeholder)} && {ruby_binary} -S bundle install"], _sudo=True)
        render("Seed placeholder Rack config", TEMPLATES / "rails/placeholder-config.ru.j2", f"{placeholder}/config.ru", user="root", group=current_ctx.runtime.runtime_group, mode="0640", **template_data(current_ctx, paths=paths))
    ...
    def validate(current_ctx, paths, ruby_binary):
        validation.run_as_runtime_user(current_ctx, "Validate Puma availability as runtime user", f"cd {quote(paths['current'])} && {ruby_binary} -S bundle exec puma --help >/dev/null")

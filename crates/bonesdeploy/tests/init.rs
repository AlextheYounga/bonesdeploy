...
fn assert_project_infra(repo: &Path) -> Result<()> {
    ...
    assert!(infra.join("provision/core/__init__.py").is_file());
    assert!(infra.join("provision/core/runtime.py").is_file());
    assert!(infra.join("provision/core/manifest.py").is_file());
...
fn rerun_preserves_existing_bones_assets() -> Result<()> {
    let env = TestEnv::new()?;
    let sentinel = env.repo().join("infra/provision/core/runtime.py");

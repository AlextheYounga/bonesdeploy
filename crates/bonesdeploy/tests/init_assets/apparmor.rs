...
fn nginx_service_template_sets_apparmor_profile() {
    let service_template = project_root().join("kit/remote/nginx/site-nginx.service.j2");
...
fn apparmor_role_assets_exist() {
    let role_root = project_root().join("kit/remote/roles/apparmor");
...
fn apparmor_role_enforces_project_profile() {
    let tasks_file = project_root().join("kit/remote/roles/apparmor/tasks/main.yml");

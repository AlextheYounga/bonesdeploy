...
fn shared_setup_deploy_keeps_runtime_roles_out() {
    ...
    assert!(!content.contains("runtimes"), "shared setup deploy must not apply runtime roles\n{content}");
...
fn remote_runtime_deploy_installs_packages_before_operations() {
    ...
    let ops_idx = content.find("get_runtime(template)");
...
fn remote_ssl_deploy_applies_ssl_operations_only() {
    ...
    assert!(!content.contains("runtimes"), "SSL deploy must not include runtime role\n{content}");
}

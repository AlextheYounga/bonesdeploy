...
pub fn run() -> Result<()> {
    ...
    fs::write(SUDOERS_PATH, &sudoers_content)
        .with_context(|| format!("Failed to write {SUDOERS_PATH}"))?;

    // Set correct permissions (sudoers drop-ins must be 0440)
    Command::new("chmod")
        .args(["0440", SUDOERS_PATH])
        .status()
        .context("Failed to chmod sudoers drop-in")?;
    ...
    println!(
        "{} Installed sudoers drop-in at {SUDOERS_PATH}",
...
fn which_gitbones_remote() -> Result<String> {
    let output = Command::new("which")
        .arg("gitbones-remote")
        .output()
        .context("Failed to run 'which gitbones-remote'")?;

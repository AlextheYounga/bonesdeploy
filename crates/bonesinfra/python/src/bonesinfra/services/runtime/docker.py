...
def deploy(ctx) -> None:
    ...
    files.directory(
        name="Create Docker PHP-FPM socket directory",
    ...
    nginx_site.render_php_fpm(
        ctx,
        paths=paths,
        template_src=ASSETS_DIR / "nginx/laravel-site-nginx.conf.j2",
    ...
    files.template(
        ...
        src=str(ASSETS_DIR / "systemd/docker-app.service.j2"),
        dest=paths.get("systemd_service", f"/etc/systemd/system/{project}-docker.service"),
        user="root",
        group="root",
        mode="0644",
...
def _build_runtime_image(ctx) -> str:
    ...
    files.template(
        ...
        src=str(ASSETS_DIR / "docker/laravel/Containerfile.j2"),
        dest=containerfile,
        user=build_user,
        group=build_user,
        mode="0640",
    ...
    files.template(
        ...
        src=str(ASSETS_DIR / "docker/laravel/www.conf.j2"),
